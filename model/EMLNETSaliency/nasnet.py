...
class NASNetALarge(nn.Module):
    ...
    def __init__(self):
        ...
        self.cell_17 = NormalCell(in_channels_left=4032, out_channels_left=672,
                                  in_channels_right=4032, out_channels_right=672)
        self.output0 = self._make_output(336)
        self.output1 = self._make_output(1344)
        self.output2 = self._make_output(2688)
        self.output3 = self._make_output(4032)
        self.combined = self._make_output(4)
    ...
    def features(self, input):
        ...
        return x_stem_1, x_reduction_cell_0, x_reduction_cell_1, x_cell_17
    ...
    def forward(self, input):
        r, c = input.size(2), input.size(3)
        out0, out1, out2, out3 = self.features(input)
        out0 = F.interpolate(self.output0(out0), (r, c), mode="bilinear")
        out1 = F.interpolate(self.output1(out1), (r, c), mode="bilinear")
        out2 = F.interpolate(self.output2(out2), (r, c), mode="bilinear")
        out3 = F.interpolate(self.output3(out3), (r, c), mode="bilinear")
        x = torch.cat([out0, out1, out2, out3], dim=1)
        x = self.combined(x)
        return x 

    def _make_output(self, planes, readout=1):
        return nn.Sequential(
            nn.Conv2d(planes, readout, 1, stride=1),
